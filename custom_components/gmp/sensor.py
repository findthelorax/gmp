...
class GMPBaseSensor(CoordinatorEntity, RestoreEntity, SensorEntity):
    def __init__(self, coordinator, name, unique):
        ...
        self._last_valid_native_value: Any | None = None
        self._has_last_valid_native_value: bool = False
    ...
    async def async_added_to_hass(self) -> None:
        ...
        restored: Any = last_state.state

        # Restore numeric sensors as numbers when possible.
        if isinstance(restored, str) and (
            self._attr_device_class in (SensorDeviceClass.ENERGY, SensorDeviceClass.MONETARY)
        ...
        self._last_valid_native_value = restored
        self._has_last_valid_native_value = True

    @property
    def available(self) -> bool:
        # Keep the entity present with its last value during transient outages/maintenance.
        return self.coordinator.last_update_success or self._has_last_valid_native_value

    def _native_value_with_cache(self, value: Any) -> Any:
        if value is None:
            return self._last_valid_native_value if self._has_last_valid_native_value else None
...
class GMPTodayEnergySensor(GMPBaseSensor):
    ...
    def native_value(self):
        ...
        return self._native_value_with_cache(data.get("today_total"))

    @property
    def extra_state_attributes(self):
        data = self.coordinator.data or {}
        hourly = data.get("hourly_values") or []
        return {
...
class GMPLastHourEnergySensor(GMPBaseSensor):
    ...
    def native_value(self):
        data = self.coordinator.data or {}
        return self._native_value_with_cache(data.get("last_hour_kwh"))
...
class GMPAccountStatusSensor(GMPBaseSensor):
    ...
    def native_value(self) -> str | None:
        data = self.coordinator.data or {}
        status: dict[str, Any] | None = data.get("status")
        if not status:
            return self._native_value_with_cache(None)

        value = "active" if status.get("active") else "inactive"
        return self._native_value_with_cache(value)
...
class GMPPowerStatusSensor(GMPBaseSensor):
    ...
    def native_value(self) -> str | None:
        data = self.coordinator.data or {}
        status: dict[str, Any] | None = data.get("status")
        return self._native_value_with_cache(_power_status(status))
...
class GMPDailyUsageSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        data = self.coordinator.data or {}
        daily: dict[str, Any] | None = data.get("daily")
        values = _usage_values(daily)
        if not values:
            return self._native_value_with_cache(None)
        return self._native_value_with_cache(_latest_numeric_any(values, ("consumed", "consumedTotal")))
...
class GMPMonthlyUsageSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        data = self.coordinator.data or {}
        monthly: dict[str, Any] | None = data.get("monthly")
...
class GMPSelectedDayTotalSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        data = self.coordinator.data or {}
        selected: dict[str, Any] | None = data.get("selected_hourly")
        ...
        for item in values:
            ...
            if isinstance(consumed, (int, float)):
                ...
                seen_any = True
        if seen_any:
            return self._native_value_with_cache(round(total, 2))

        # Fallback: Some responses only include cumulative totals.
        consumed_total = _latest_numeric_any(values, ("consumedTotal",))
        value = round(consumed_total, 2) if consumed_total is not None else None
        return self._native_value_with_cache(value)
...
class GMPEVEnergyPeriodConsumptionSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        data = self.coordinator.data or {}
        ev_daily: dict[str, Any] | None = data.get("ev_daily")
        interval = _ev_interval(ev_daily)
        if not interval:
            return self._native_value_with_cache(None)
...
class GMPEVSelectedDayConsumptionSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        ...
        if not item:
            return self._native_value_with_cache(None)
        consumed = item.get("consumed")
        value = float(consumed) if isinstance(consumed, (int, float)) else None
        return self._native_value_with_cache(value)
...
class GMPEVSelectedDayCostSensor(GMPBaseSensor):
    ...
    def native_value(self) -> float | None:
        ...
        if not item:
            return self._native_value_with_cache(None)
        cost = item.get("cost")
        value = float(cost) if isinstance(cost, (int, float)) else None
        return self._native_value_with_cache(value)
